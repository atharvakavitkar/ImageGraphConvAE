...
def get_data_loaders(transform):
    ...
    train_loader = torch.utils.data.DataLoader(circuit_trainset, batch_size=batchsize, shuffle=True,pin_memory=True,num_workers = 20)
...
def train(train_loader):
    ...
    for epoch in range(num_epochs):
        ...
        for image_batch in train_loader:
            ...
            # vae reconstruction
            image_recon_batch,latent_batch = model(image_batch.cuda())

            # reconstruction error
            loss = criterion(image_recon_batch.cuda(), image_batch.cuda())
        ...
        epoch_end = time.time()
        print('Epoch [%d / %d] average reconstruction error: %f \t Time taken:%f' % (epoch+1, num_epochs, train_loss_avg[-1], epoch_end-epoch_start))
        training_outputs.append([image_batch[9], image_recon_batch[9]])
        if(epoch%100 == 0):
            torch.save(model,'' + str(epoch) + ".h5")
    return train_loss_avg,training_outputs
...
if __name__=='__main__':
    ...
    train_loader = get_data_loaders(transform)  #,valid_loader,test_loader

    train_start = time.time()
    train_loss_avg,training_outputs = train(train_loader)
    train_end = time.time()
